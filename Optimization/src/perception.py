...
def get_detectron_masks(image, predictor, classes=None, expansion=(1., 1.), debug=False):
    ...
    # plt.show()
    # cv2.imwrite("SegLab.jpg", out.get_image()[:, :, ::-1])
...
if __name__ == "__main__":
    ...
    image = cv2.imread('/home/falah/Desktop/distortion-free-wide-angle.pytorch-main/data/1_150.bmp')
    ...
    dlib_detector = dlib.cnn_face_detection_model_v1('/home/falah/Desktop/distortion-free-wide-angle.pytorch-main/data/mmod_human_face_detector.dat')

    box_masks, seg_masks = get_detectron_masks(image, detectron, classes=None)
    box_masks = get_dlib_masks(image, dlib_detector)
    ...
    plt.show()
    cv2.imwrite("Facebox.jpg", get_overlay_mask(image, box_mask)[:, :, ::-1])

    plt.figure(figsize=(10, 8))
    plt.imshow(get_overlay_mask(image, seg_mask)[:, :, ::-1])
    plt.show()
    cv2.imwrite("SegMap.jpg", get_overlay_mask(image, seg_mask)[:, :, ::-1])

    plt.figure(figsize=(10, 8))
    plt.imshow(get_overlay_mask(image, joint_mask)[:, :, ::-1])
    plt.show()
    cv2.imwrite("Join.jpg", get_overlay_mask(image, joint_mask)[:, :, ::-1])
