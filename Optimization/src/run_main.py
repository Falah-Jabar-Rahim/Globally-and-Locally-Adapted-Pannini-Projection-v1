...
# from Optimization.src.data import ImageDataset
...
def optimize_mesh(self):
    ...
    # filename, _ = os.path.splitext(filename)   # original

    mesh_uniform_padded, mesh_stereo_padded, correction_strength, seg_mask_padded, box_masks_padded = data.get_image_by_file(
        self)


    return mesh_uniform_padded, mesh_stereo_padded,  correction_strength, seg_mask_padded, box_masks_padded


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Distortion-Free-Wide-Angle-Portraits-on-Camera-Phones')
    parser.add_argument('--file', type=str, default='/home/falah/Desktop/Pannini/Optimization/data/1_150.bmp')
    ...
    parser.add_argument('--mesh_ds_ratio', type=int, default=30, help="the pixel-to-vertex ratio")

    parser.add_argument('--naive', type=int, default=1, help="if set True, perform naive orthographic correction")
    parser.add_argument('--face_energy', type=float, default=5, help="weight of the face energy term")
    parser.add_argument('--similarity', type=int, default=1, help="weight of similarity tranformation constraint")
    parser.add_argument('--line_bending', type=float, default=3, help="weight of the line bending term")
    parser.add_argument('--regularization', type=float, default=0.5, help="weight of the regularization term")
    parser.add_argument('--boundary_constraint', type=float, default=5, help="weight of the mesh boundary constraint")
