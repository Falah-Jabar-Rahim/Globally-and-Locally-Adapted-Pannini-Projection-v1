...
class Energy(nn.Module):
    ...
    def forward(self):
        ...
        object_energy = masks * self.correction_strength * (self.mesh - target_mesh) ** 2
        similarity_regularizer = (self.similarity[:, 0] - 1.) ** 2
        object_energy = object_energy.mean()
        if self.opt["similarity"]: object_energy += 1e4 * similarity_regularizer.mean()
        ...
        energy = self.opt["object_energy"] * object_energy \
